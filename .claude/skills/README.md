...
Catalog of the skills in this repo, tracked in git under `.claude/skills/`
(the `.claude/` directory is otherwise git-ignored local state — see
`.gitignore`). Claude Code exposes each one by its **directory name** — use
that name exactly.
...
under `.claude/skills/speckit-*`, `.specify/templates/`, `.specify/scripts/`, and
...
at container-create time, via `specify integration upgrade claude` (native Claude
Code integration — it writes these skills directly, no symlinking needed). The
pinned version lives in `.specify/speckit.lock` (tracked); a rebuild that changes
it bumps the lock and prints a notice so the transition gets reviewed and
committed. `.specify/memory/` (the constitution) is hand-owned and never
regenerated.
...
| `wiki-maintenance` (agent) | Audits the generated wiki: stale pages, unmatched/uncovered `sources.yaml` globs, index links, dead citations, oversized pages. Flags for a librarian run; never regenerates pages itself. See `.claude/agents/wiki-maintenance.md`. |
