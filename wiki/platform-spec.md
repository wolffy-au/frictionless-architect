...
sources:
  ...
  - specs/002-neo4j-schema-ui/checklists/requirements.md
---
...
One edge case is now marked resolved: schema files on a newer ArchiMate
version (3.1) with sample data still on 3.0-style nodes. Because the 3.1
exchange-format XSDs keep the 3.0 namespace, 3.0-namespaced samples stay
valid. A sample in any other namespace yields a warning naming the expected
namespace and `sample_file_status: "invalid"`, instead of rendering an empty
model (`specs/002-neo4j-schema-ui/spec.md:66`, citing ADR-0032). The plan's
target-platform line now runs the app as `uvicorn frictionless_architect.app:app`
(`specs/002-neo4j-schema-ui/plan.md:15`).
...
parsed. `missing` means the file is absent or not well-formed XML. `invalid`
means the root is not an ArchiMate `<model>` in the
`http://www.opengroup.org/xsd/archimate/3.0/` namespace, and `last_warning`
carries the details (`specs/002-neo4j-schema-ui/contracts/api.md`
§"`/schema-payload/status`"; ADR-0032, see [Data Model](data-model.md)). The
contract lists only the three JSON endpoints. The server-rendered
`/schema-visualizer` HTML page was removed from the app (see
[Visualizer Service](visualizer-service.md)).
...
`specs/002-neo4j-schema-ui/checklists/requirements.md` is the Spec Kit
quality gate the spec passed before planning. It was created on 2026-04-04,
the same date as the spec. Every item is ticked across its three groups:

- **Content quality**: no implementation details, and written for
  non-technical stakeholders.
- **Requirement completeness**: no `[NEEDS CLARIFICATION]` markers left;
  requirements are testable; success criteria are measurable and
  technology-agnostic; edge cases, scope and dependencies are identified.
- **Feature readiness**: every functional requirement has acceptance
  criteria.

(`specs/002-neo4j-schema-ui/checklists/requirements.md:7-30`)

The checklist is a one-time snapshot. The spec itself is still marked
`Status: Draft` (`specs/002-neo4j-schema-ui/spec.md:5`), and the "no
implementation details" ticks predate the later edits that name ADR-0032 and
the ArchiMate namespace. The checklist has not been re-run since, so it
doesn't say whether the spec still passes.
