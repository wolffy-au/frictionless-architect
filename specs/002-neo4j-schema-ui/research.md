...
**Decision:** Use `defusedxml`-wrapped `xml.etree.ElementTree` to load `sample-data/schema/*.xsd` and `sample-data/sample-00/Test Model Full.xml`, extract element/relationship/view definitions, and normalize them into JSON payloads that both Neo4j ingestion and the visualiser share; validate the sample against the XSDs (entry point `archimate3_Diagram.xsd`, which includes the View and Model schemas) with `xmlschema`, offline and `defusedxml`-hardened, surfacing violations as non-blocking payload warnings. See [ADR-0022](../../docs/adr/0022-schema-visualiser-lxml-xmlschema.md).  
**Rationale:** `xmlschema` understands XSD-defined structures so we can validate the sample data against the schema files; `defusedxml`/`ElementTree` keeps parsing fast for the small sample data while also hardening against XXE and entity-expansion attacks, with no `lxml` C-extension dependency. Normalizing into JSON simplifies the front-end contract and lets us detect coverage gaps (schema entry without matching sample element) while still providing the layout bounds FR-003 needs.  
